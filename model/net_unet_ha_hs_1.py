...
class Net(nn.Module):
    def __init__(self, train_size, test_size, is_high, post_processing):
        ...
        # self.a_model = analysisTransformModel(3, [N, N, N, N])
        # s_model 为解码器
        self.s_model = synthesisTransformModel(N, [N, N, N, 3])
        ...
        self.h_s_scale = Unet_hs_new(192, 8, 3)
        self.h_s_means = Unet_hs_new(192, 8, 3)
    ...
    def base_params(self):
        ...
        params += self.h_s_scale.parameters()
        params += self.h_s_means.parameters()
    ...
    def forward(self, inputs, mode='train', num=1):
        ...
        latent_scales = self.h_s_scale(z_hat,middle_x,down_x1 ,input)  # 8 * 320 * 16 * 16
        latent_means = self.h_s_means(z_hat,middle_x,down_x1 ,input)  # 8 * 320 * 16 * 16
