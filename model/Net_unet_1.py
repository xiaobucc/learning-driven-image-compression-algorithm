# 超先验网络用unet网络替代。 这里我将前面通道的信息转为Transformer   后面通道的信息转为卷积层，res块用卷积层替代
import os
...
#
# @torch.no_grad()
# def denoise(latents): # 1 * 4 * 64 * 64
#     latents = latents * 0.18215
#     step_size = 15
#     num_inference_steps = scheduler.config.get("num_train_timesteps", 1000) // step_size # 66步
#     strength = 0.04
#     scheduler.set_timesteps(num_inference_steps)
#     offset = scheduler.config.get("steps_offset", 0)
#     init_timestep = int(num_inference_steps * strength) + offset # 2  # 2
...
#     input_chunks = torch.chunk(latents, 8, dim=0)
#     out_put = []
#     with autocast():
#         for b in range(latents.shape[0]):
#             input = input_chunks[b]
#             # print("input:{}".format(input.shape))
#             for i, t in enumerate(scheduler.timesteps[t_start:]):
#                 noise_pred = unet(input, t, encoder_hidden_states=uncond_embeddings).sample
#                 # print("noise_pred:{}".format(noise_pred.shape))
#                 latents = scheduler.step(noise_pred, t, input, **extra_step_kwargs).prev_sample
#                 latents = latents / 0.18215
#                 # print("latents:{}".format(latents.shape))
#             out_put.append(latents)
#     # reset scheduler to free cached noise predictions
#     scheduler.set_timesteps(1)
#     out_put = torch.cat(out_put,dim=0)
#     return out_put
...
class Net(nn.Module):
    def __init__(self, train_size, test_size, is_high, post_processing):
        ...
        self.unet = Unet_new(192,192,8,3)
        # 初始化超先验网络的参数
    ...
    def base_params(self):
        ...
        params += self.cit_he.parameters()
        params += self.cit_hd.parameters()
    ...
    def forward(self, inputs, mode='train', num=1):
        ...
        y_shape = z3.shape[2:]
        # image_path_ga = "/media/yang/Pytorch/buxiaobu/code/Neural_Syntax/test_kokal/ga"
        # visualization_FeatureMap(inputs,self.a_model,192,image_path_ga,0)

        # z2 = self.CTM_ha(z3)
        # image_path_ha = "/media/yang/Pytorch/buxiaobu/code/Neural_Syntax/test_kokal/ha"
        # visualization_FeatureMap(z3,self.CTM_ha,192,image_path_ha,0)
        #
        # _, z_likelihoods = self.entropy_bottleneck(z2)  # 8 * 192 * 4 * 4
        # z_offset = self.entropy_bottleneck._get_medians()  # 8 * 192 * 1 * 1
        # z_tmp = z2 - z_offset
        # # 对z2_rounded进行反卷积操作
        # # torch.Size([8, 192, 4, 4])--> torch.Size([8, 192, 16, 16])
        # #  从超先验网络中出来
        # z2_noisy = self.quant_noise(z_tmp)
        # z2_rounded = bypass_round(z2_noisy)
        # h2 = self.cit_hd(z2_rounded)
        # h2 = self.hs_model(z2_rounded)


        latent_scales = self.unet(z3)
        latent_means = self.unet(z3)  # 8 * 192 * 16 * 16
        ...
        if mode == 'train':
            ...
            # v_psnr = -10 * math.log10(v_mse)
            return bpp_list, v_mse, v_psnr


if __name__ == "__main__":
    x = torch.randn(size=(8, 192, 16, 16))
    y_rec = denoise(x)
