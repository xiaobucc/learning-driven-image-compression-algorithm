...
class WinBasedAttention(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        shortcut = x
        x = x.permute(0, 2, 3, 1)   # 2 *  8 * 8 * 192


        if self.shift_size > 0:  # shift_size = 2
...
if __name__ == '__main__':
    x = torch.rand([2, 192, 8, 8])
    attn = WinBasedAttention(dim=192, num_heads=8, window_size=4, shift_size=2)
